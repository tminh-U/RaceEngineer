...
{
    connect(this, &MessageDispatcher::requestSpeak, backend_, &ITtsBackend::speak, Qt::QueuedConnection);
    connect(this, &MessageDispatcher::requestStop, backend_, &ITtsBackend::stop, Qt::QueuedConnection);
    connect(backend_, &ITtsBackend::speakingFinished, this, [this] {
        speaking_ = false;
...
{
    if (text.trimmed().isEmpty() || !backend_->isAvailable()) return;
