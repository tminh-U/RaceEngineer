...
constexpr wchar_t raceDataShareTarget[] = L"RaceEngineer/RaceDataShareToken";
}
...
QString CredentialStore::readRaceDataShareToken()
{
#ifdef _WIN32
    PCREDENTIALW credential = nullptr;
    if (CredReadW(raceDataShareTarget, CRED_TYPE_GENERIC, 0, &credential) == FALSE) return {};
    const QByteArray bytes(reinterpret_cast<const char*>(credential->CredentialBlob),
                           static_cast<qsizetype>(credential->CredentialBlobSize));
    CredFree(credential);
    return QString::fromUtf8(bytes);
...
bool CredentialStore::writeRaceDataShareToken(const QString& token)
{
    if (token.isEmpty()) return clearRaceDataShareToken();
#ifdef _WIN32
    const QByteArray bytes = token.toUtf8();
    CREDENTIALW credential{};
    credential.Type = CRED_TYPE_GENERIC;
    credential.TargetName = const_cast<wchar_t*>(raceDataShareTarget);
    credential.CredentialBlobSize = static_cast<DWORD>(bytes.size());
    credential.CredentialBlob = reinterpret_cast<LPBYTE>(const_cast<char*>(bytes.constData()));
    credential.Persist = CRED_PERSIST_LOCAL_MACHINE;
    credential.UserName = const_cast<wchar_t*>(L"RaceEngineer");
...
bool CredentialStore::clearRaceDataShareToken()
{
#ifdef _WIN32
    if (CredDeleteW(raceDataShareTarget, CRED_TYPE_GENERIC, 0) != FALSE) return true;
    return GetLastError() == ERROR_NOT_FOUND;
#else
    return false;
#endif
