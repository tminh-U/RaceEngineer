...
// Assetto Corsa graphics/session page. This page supplies lap timing, race
// position, flags and pit state; none of those values are present in physics.
struct SPageFileGraphic
{
	int packetId = 0;
	AC_STATUS status = AC_OFF;
	AC_SESSION_TYPE session = AC_UNKNOWN;
...
#ifdef _WIN32
static_assert(sizeof(SPageFileGraphic) == 284, "Unexpected AC graphics-page layout");
#endif
